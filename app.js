...
const resultReasonCopy = {
  activity: { good: 'Породе подходит выбранное вами время на прогулки', hard: (d) => d.breed > d.user ? 'Ей может требоваться больше прогулок, чем вы готовы предложить' : 'Её потребность в прогулках может быть ниже желаемой вами' },
  mentalWork: { good: 'Ей подойдёт выбранный вами объём занятий', hard: (d) => d.breed > d.user ? 'Ей может требоваться больше игр и занятий' : 'Ей может быть не нужен такой объём занятий, как вы хотите' },
  biddability: { good: 'Её управляемость соответствует вашим ожиданиям', hard: 'Она может не так быстро и охотно выполнять команды, как вы ожидаете' },
  independence: { good: 'Её самостоятельность будет вам комфортна', hard: 'Её самостоятельность может оказаться для вас неудобной' },
  recall: { good: 'От неё можно ожидать подходящий вам уровень отзыва после обучения', hard: 'Надёжный отзыв на прогулке может потребовать больше работы' },
  preyDrive: { good: 'Её интерес к погоне укладывается в допустимый для вас уровень', hard: 'Желание преследовать животных может быть сильнее, чем вам комфортно' },
  guarding: { good: 'Её сторожевое поведение соответствует вашим ожиданиям', hard: 'Её реакция на посторонних может быть для вас слишком выраженной' },
  strangerFriendliness: { good: 'Её отношение к гостям будет вам комфортно', hard: 'С гостями она может вести себя не так, как вам хотелось бы' },
  dogSociability: { good: 'Её поведение рядом с другими собаками соответствует вашим ожиданиям', hard: 'Встречи с другими собаками могут требовать больше контроля' },
  affection: { good: 'Она подходит вам по потребности в общении дома', hard: 'Её потребность в близком общении дома может вам не подойти' },
  offSwitch: { good: 'После прогулки ей будет достаточно легко отдыхать дома', hard: 'Дома ей может быть сложнее успокоиться, чем вы ожидаете' },
  sensitivity: { good: 'Её чувствительность к шуму будет вам комфортна', hard: 'Шум и неожиданные события могут вызывать слишком сильную реакцию' },
  vocality: { good: 'Её голосистость укладывается в приемлемый для вас уровень', hard: 'Она может лаять или выть чаще, чем вам комфортно' },
  grooming: { good: 'Уход за её шерстью соответствует вашим возможностям', hard: 'Уход за шерстью может требовать больше времени и денег' },
  reactivity: { good: 'Её поведение на поводке соответствует вашим ожиданиям', hard: 'На поводке её реакции могут требовать регулярного контроля' },
  shedding: { good: 'Уровень линьки будет для вас приемлемым', hard: 'В доме может быть больше шерсти, чем вам хотелось бы' },
  drool: { good: 'Уровень слюноотделения будет для вас приемлемым', hard: 'Слюноотделение может быть сильнее, чем вам комфортно' },
  power: { good: 'Её физическая сила будет для вас комфортна', hard: 'Её может быть сложно физически удержать на поводке' },
  healthBurden: { good: 'Породные риски для здоровья укладываются в допустимый для вас уровень', hard: 'Породные особенности здоровья могут потребовать повышенного внимания и расходов' }
};
function humanReasons(details, direction) {
  return Object.entries(details)
    .sort((a, b) => direction === 'good' ? b[1].match - a[1].match : a[1].match - b[1].match)
    .slice(0, direction === 'good' ? 4 : 3)
    .map(([trait, detail]) => {
      const copy = resultReasonCopy[trait]?.[direction];
      return typeof copy === 'function' ? copy(detail) : copy;
    })
    .filter(Boolean);
}
...
function commonResultSections(includeProfile = true) {
  ...
  return `${profileSection}<section class="result-section next-steps"><h2>Что делать дальше</h2><div class="next-grid"><article class="result-card"><h3>Чек-лист до появления собаки</h3><ul class="result-list"><li>Познакомиться с конкретной собакой или родителями щенка.</li><li>Проверить документы, условия содержания и обследования.</li><li>Подготовить лежанку, миски, корм, игрушки, шлейку и обычный поводок.</li></ul></article><article class="result-card consultation-card"><h3>Консультация с кинологом</h3><p>Разберём ваши ответы, образ жизни и несколько конкретных пород до решения.</p><button class="button" id="consultation-button" type="button">Оставить заявку</button></article></div></section><dialog class="consultation-dialog" id="consultation-dialog"><button class="consultation-close" type="button" aria-label="Закрыть форму">×</button><h2>Заявка на консультацию</h2><p>Оставьте Telegram или номер телефона и коротко опишите, с чем нужна помощь.</p><form action="https://formsubmit.co/kuznetsov1egor@gmail.com" method="POST"><input type="hidden" name="_subject" value="Новая заявка с сайта Мой пёс"><input type="hidden" name="_template" value="table"><input type="hidden" name="_captcha" value="false"><input type="hidden" name="Страница" value="${window.location.href}"><input id="consultation-profile" type="hidden" name="Профиль пользователя" value=""><label>Telegram или телефон<input name="Контакт" type="text" placeholder="@username или +7 900 000-00-00" required></label><label>Ваш запрос<textarea name="Запрос" rows="5" placeholder="Например: выбираю между биглем и уиппетом, дома живёт кошка" required></textarea></label><button class="button" type="submit">Отправить заявку</button><p class="consultation-privacy">Нажимая кнопку, вы соглашаетесь передать эти данные для связи по заявке.</p></form></dialog><section class="result-section result-card"><h2>Жизнь с собакой</h2><p class="section-note">Даже хорошо подобранная собака остаётся собакой. Это ежедневные прогулки, расходы на корм и ветеринара, организация поездок, обучение, грязные лапы и иногда испорченные вещи. Щенок может первое время ходить в туалет дома, будить ночью, грызть вещи и плохо оставаться один. Взрослой собаке тоже понадобится время на адаптацию. Хороший подбор делает совместную жизнь проще, но не делает собаку беспроблемной.</p><p class="table-note">Породный профиль описывает тенденции, а не гарантирует характер конкретной собаки.</p></section><section class="result-section bri-support"><img src="assets/egor-and-bri.png" alt="Егор и Бри"><div><h2>На новую игрушку Бри</h2><p>Тест останется бесплатным. Если он оказался полезен, можно поддержать проект.</p><a class="button" href="${donationUrl}" target="_blank" rel="noopener" data-donation="result">Поддержать проект</a></div></section>${footer()}`;
}
...
function bindConsultation() {
  ...
  const profile = document.querySelector('#consultation-profile');
  if (profile) {
    const resultTitle = document.querySelector('.result-hero h1')?.textContent.trim().replace(/\s+/g, ' ') || 'Результат не определён';
    const preferences = [...document.querySelectorAll('.result-section .result-list li')].map((item) => item.textContent.trim()).join(' | ');
    profile.value = `${resultTitle}. ${preferences}`;
  }
  if (button && dialog) button.addEventListener('click', () => {
    analytics.goal('consultation_open');
    dialog.showModal();
  });
